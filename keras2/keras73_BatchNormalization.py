...
# Regularization은 W(weight)가 너무 큰 값들을 갖지 않도록 하는 것을 말합니다. 
# 값이 커지게 되면 구불구불한 형태의 코스트함수가 만들어지고 예측에 실패하게 되는데, 이를 머신러닝에서는 “데이터보다 모델의 복잡도(Complexity)가 크다” 라고 설명합니다.
...
# L1 regularization : 대부분의 요소값이 0인 sparse feature에 의존한 모델에서 불필요한 feature에 대응하는 가중치를 0으로 만들어 
# 해당 feature를 모델이 무시하게 만듬. feature selection효과가 있다.

# L2 regularization : 아주 큰 값이나 작은 값을 가지는 outlier모델 가중치에 대해 0에 가깝지만 0은 아닌값으로 만듬. 
# 선형모델의 일반화능력을 개선시키는 효과가 있다.
